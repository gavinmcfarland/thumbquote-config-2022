// We create a main function, so we can use the await keyword
async function main() {
   ...
   // We check the fontName is not a mixed value so that we don't get a TypeScript error. You can try removing this and re-running the plugin (don't forget to rebuild), you'll see that despite getting an error in the console, the plugin will still work. This is beacuse TypeScript is designed to improve the quality of your code, but it doesn't interfere with what you're building.
   ...
   await figma.setFileThumbnailNodeAsync(frameNode)

   // Now we can close the plugin
   figma.closePlugin("Thumbquote set")
}

// We need to call the main function so our plugin code runs
main()
